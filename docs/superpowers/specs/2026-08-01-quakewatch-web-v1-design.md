...
- **Mappa**: epicentri come cerchi — raggio ∝ magnitudo, recenza in accento rosso che sfuma verso il grigio con l'età; pulse sull'ultimo evento in live; selezione sincronizzata lista↔mappa. Niente cluster in v1.
  > **Revisione 2026-08-02 (round dataviz):** codifica marker **bivariata** — colore = classe di magnitudo (convenzione INGV giallo→rosso, deroga dichiarata alla disciplina monocromo: è codifica dati, non skin), opacità = età. **Legenda obbligatoria** (magnitudo + età + selezione, e scala MMI distinta quando attivo il layer ShakeMap). Il rosso accento resta per selezione/LIVE; la resa della selezione con i marker colorati si definisce col supporto della skill dataviz. Riferimenti: `docs/inspiration/dashboards/`. Dettaglio evento come riquadro flottante su mappa (desktop): da validare in A/B contro il comportamento attuale in sidebar; su mobile resta il sheet.
