...
describe('buildEventsUrl', () => {
  ...
  it('applica minmagnitude=2 alle finestre lunghe', () => {
    expect(
      buildEventsUrl(BASE, '30d', italia, now).searchParams.get('minmagnitude')
    ).toBe('2')
    expect(
      buildEventsUrl(BASE, '90d', italia, now).searchParams.get('minmagnitude')
    ).toBe('2')
  })

  it('stesso minuto → stessa URL (chiave di cache condivisa)', () => {
    const a = buildEventsUrl(
      BASE,
      '7d',
      italia,
      new Date('2026-08-01T10:23:05Z')
    )
    const b = buildEventsUrl(
      BASE,
      '7d',
      italia,
      new Date('2026-08-01T10:23:55Z')
    )
