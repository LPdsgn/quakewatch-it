...
const origin = (
  id: string,
  mode: OriginRevision['evaluationMode']
): OriginRevision => ({
  publicId: id,
...
describe('revisionStatus', () => {
  it('origin preferita manuale → rivisto', () => {
    expect(
      revisionStatus(
        detail([origin('a', 'automatic'), origin('b', 'manual')], [mag('m', 2)])
      )
    ).toBe('rivisto')
  })
  ...
  it('origin preferita automatica → preliminare', () => {
    expect(
      revisionStatus(detail([origin('a', 'automatic')], [mag('m', 2)]))
    ).toBe('preliminare')
  })

  it('evaluationMode assente → preliminare (prudenza)', () => {
    expect(revisionStatus(detail([origin('a', null)], [mag('m', 2)]))).toBe(
      'preliminare'
    )
  })
})
...
describe('hasRevisions', () => {
  it('true con più di una origin o magnitudo', () => {
    expect(
      hasRevisions(
        detail([origin('a', null), origin('b', null)], [mag('m', 2)])
      )
    ).toBe(true)
    expect(
      hasRevisions(detail([origin('a', null)], [mag('m', 2), mag('n', 2.2)]))
    ).toBe(true)
  })
