...
const xml = readFileSync(
  fileURLToPath(
    new URL('./fixtures/event-detail.quakeml.xml', import.meta.url)
  ),
  'utf8'
)
...
describe('parseQuakemlEvent', () => {
  it('estrae il dettaglio con tutte le revisioni dalla fixture reale', () => {
    ...
    expect(d.origins.map((o) => o.publicId)).toContain(
      d.preferredOrigin.publicId
    )
    expect(d.magnitudes.map((m) => m.publicId)).toContain(
      d.preferredMagnitude.publicId
    )
