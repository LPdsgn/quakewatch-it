...
- Persistenza stato UI: **view in URL, preferenze in localStorage**. View (`window`/`area`/`event`/`t`) = condivisibile, deep-linkable, server-renderable, back/forward; preferenze (`variant`, future `basemap`/`stations`/`expert`) = localStorage via hook `usePersistentPref<T>(key, default)` in `packages/core`, mai cookie (inviati al server a ogni request + rischio consenso ePrivacy; localStorage rientra nell'esenzione strictly-necessary, no banner). Theme resta su next-themes. URL può fare override temporaneo di una preferenza (deep link `?basemap=terrain`)
...
- Animazioni: **anime.js v4** (revisione 2026-08 della scelta GSAP: bundle −38% misurato, motion sobrio da spec), non framer-motion né GSAP. Componenti React di terze parti che animano con altre librerie si migrano ad anime.js o si scartano. Hook React interno su `createScope`; skill in `.agents/skills/animejs` (vendorizzata, pinnata in `skills-lock.json`)
